...
export default function BackOffice({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (isAdmin && tab === "prompts") loadPrompts(selectedAgent);
  }, [selectedAgent, tab, isAdmin]);

  useEffect(() => {
    if (isAdmin && tab === "status") loadStatuses();
  }, [tab, isAdmin]);
  ...
  if (!isAdmin) {
    return (
      <div style={{ width: "100vw", height: "100vh", background: "#050507", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Inter',sans-serif" }}>
        ...
        <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 14, letterSpacing: 1.5, marginBottom: 24 }}>ACCESS DENIED</div>
        <button onClick={onBack} style={{ background: "transparent", border: `1px solid ${OR(0.3)}`, color: OR(0.7), padding: "8px 20px", borderRadius: 6, cursor: "pointer", fontSize: 12, letterSpacing: 1 }}>← Back</button>
      </div>
    );
  }
